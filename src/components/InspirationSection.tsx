      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {inspirations.map((inspiration, index) => (
            <div 
              key={inspiration.id}
              className="brand-card bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all group opacity-0 translate-y-10"
        ...
        <div className="text-center mt-12 animate-fade-in" style={{ animationDelay: '500ms' }}>
          <Link to="/brand">
            <Button className="bg-brand-blue hover:bg-brand-blue/90 transition-transform hover:scale-105 duration-200">
              {t('brand.viewAll')}
            </Button>
          </Link>
        </div>
